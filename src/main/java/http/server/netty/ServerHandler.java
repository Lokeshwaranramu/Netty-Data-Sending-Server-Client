...
public class ServerHandler extends ChannelInboundHandlerAdapter
{
    ...
    private void handleHttpRequest(ChannelHandlerContext ctx, FullHttpRequest req) {
        ...
        if (parameters.size() == 0 || !parameters.containsKey(HTTP_REQUEST_STRING)) {
            System.err.printf(HTTP_REQUEST_STRING + "UFF!!!");
            sendHttpResponse(ctx, req, new DefaultFullHttpResponse(HTTP_1_1, NOT_FOUND));
            return;
        }
        ...
        if (handshaker == null) {
            ...
            if (channelFuture.isSuccess()) {
                if (client.getId() == 0) {
                    System.out.println(ctx.channel() + "yep!!");
                    return;
                }
    ...
    private void broadcast(ChannelHandlerContext ctx, WebSocketFrame frame) {
        ...
        if (client.getId() == 0) {
            Response response = new Response(100, "Good to see that");
        ...
        System.out.println(" Hi " + ctx.channel() + request);
    ...
    public void handlerAdded(ChannelHandlerContext ctx) throws Exception {
        ...
        System.out.println("the data is from" + incoming.remoteAddress() + " address");
    }
